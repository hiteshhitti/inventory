...
class PurchaseCreateView(View):                                                 
    ...
    def post(self, request, pk):
        ...
        if formset.is_valid():
            ...
            # 🔹 Create Purchase Bill properly
            billobj = PurchaseBill.objects.create(
                supplier=supplierobj
            )

            # 🔹 Create Bill Details
            billdetailsobj = PurchaseBillDetails.objects.create(
                billno=billobj,
                total=0
            )
...
class SaleCreateView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid() and formset.is_valid():
            ...
            billdetailsobj = SaleBillDetails.objects.create(
                billno=billobj,
                total=0
            )
            ...
            for item_form in formset:
                if item_form.cleaned_data:
                    ...
                    if stock.quantity < billitem.quantity:
                        messages.error(request, f"Not enough stock for {stock.name}")
